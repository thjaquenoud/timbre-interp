...
from tensorflow.keras.layers import Input, Dense, Conv1D, Conv1DTranspose, Flatten, Lambda, Concatenate, Dropout, LeakyReLU, Multiply, Add
...
class Manne:
	...
	def vae_loss(self, encoded_mean, encoded_log_sd, inputs, outputs):
		mse_loss = mse(inputs, outputs)
		kl_loss = 1 + encoded_log_sd - K.square(encoded_mean) - K.exp(encoded_log_sd)
	...
	def define_net(self):
		def sampling(params):
			...
			return mean + tf.exp(log_sd) * epsilon

		l2_penalty = 1e-7

		self.encoder_widths = [256,128,64,32,16,10]
		self.decoder_widths = [16,32,64,128,256]
		self.cnn_filters = [4,2]

		self.decoder_outdim = 2049
		drop = 0.0
		alpha_val=0.1
		input_spec1 = Input(shape=(decoder_outdim,))

		encoded1 = tf.expand_dims(input_spec1,axis=2)
		encoded1 = Conv1D(input_shape=(-1,self.decoder_outdim,1), filters=self.cnn_filters[0], kernel_size=3, kernel_regularizer=l2(l2_penalty))(encoded1)
		encoded1 = LeakyReLU(alpha=alpha_val)(encoded1)

		encoded1 = Conv1D(self.cnn_filters[1], 3, kernel_regularizer=l2(l2_penalty))(encoded1)
		encoded1 = LeakyReLU(alpha=alpha_val)(encoded1)
		...
		encoded_resampler = Input(shape=(self.encoder_widths[-2],))
		encoded_mean = Dense(units=self.encoder_widths[-1], kernel_regularizer=l2(l2_penalty))(encoded_resampler)
		encoded_log_sd = Dense(units=self.encoder_widths[-1], kernel_regularizer=l2(l2_penalty))(encoded_resampler)
		resampled = Lambda(sampling)([encoded_mean, encoded_log_sd])
		self.resampler = Model(inputs=[encoded_resampler], outputs=[encoded_mean, encoded_log_sd, resampled])
		...
		decoded1 = Dense(units=self.decoder_outdim-4,
			activation=None,
			kernel_regularizer=l2(l2_penalty))(decoded1)
		decoded1 = LeakyReLU(alpha=alpha_val)(decoded1)

		decoded1 = tf.expand_dims(decoded1,axis=2)
		decoded1 = Conv1DTranspose(self.cnn_filters[1], 3, kernel_regularizer=l2(l2_penalty))(decoded1)
		decoded1 = LeakyReLU(alpha=alpha_val)(decoded1)

		decoded1 = Conv1DTranspose(1, 3,activation='relu', kernel_regularizer=l2(l2_penalty))(decoded1)
		decoded1 = tf.squeeze(decoded1, axis=2)

		self.decoder = Model(inputs=[input_latent1], outputs=[decoded1])

		my_batch = [input_spec1]
		my_encoded = self.encoder(my_batch)
		my_mean, my_log_sd, my_resampled = self.resampler(my_encoded)
		my_decoded = self.decoder(my_resampled)
		self.network = Model(inputs=my_batch, outputs=my_decoded)

		self.network.add_loss(self.vae_loss(my_mean, my_log_sd, my_batch, my_decoded))
	...
	def train_net(self):
		...
		mod = Input(shape=(self.encoder_widths[-1],))
		final_spec_1 = Input(shape=(self.decoder_outdim,))
		my_batch0 = [final_spec_1]

		my_encoded0 = self.encoder(my_batch0)
		my_encoded0 = Dense(units=self.encoder_widths[-1], kernel_regularizer=l2(l2_penalty))(my_encoded0)
		blarg0 = Multiply()([my_encoded0,mod])
		final_decoded = self.decoder([blarg0])

		self.dnb_net = Model(inputs=[final_spec_1,mod],
	...
	def evaluate_net(self):
		...
		a=self.network.evaluate(x=val_data,verbose=1)
		b=self.network.evaluate(x=test_data,verbose=1)
		print('\n')
