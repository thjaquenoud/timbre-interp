...
class Manne:
	...
	def define_net(self):
		...
		drop = 0.0
		alpha_val=0.1
		input_spec1 = Input(shape=(decoder_outdim,), name="Encoder_Input")

		encoded1 = Dense(units=self.encoder_widths[0],
				activation=None,
				kernel_regularizer=l2(l2_penalty),
				name="Encoder_Dense1")(input_spec1)

		encoded1 = LeakyReLU(alpha=alpha_val, name="Encoder_LeakyRelu1")(encoded1)
		for i,width in enumerate(self.encoder_widths[1:-1]):
			encoded1 = Dense(units=width,
				activation=None,
				kernel_regularizer=l2(l2_penalty),
				name="Encoder_Dense" + str(i + 2))(encoded1)
			encoded1 = LeakyReLU(alpha=alpha_val, name="Encoder_LeakyRelu" + str(i + 2))(encoded1)

		encoded1 = Dense(units=self.encoder_widths[-1], activation='sigmoid', kernel_regularizer=l2(l2_penalty), name="Encoder_Dense" + str(len(self.encoder_widths) + 1))(encoded1)
		self.encoder = Model(inputs=[input_spec1], outputs=[encoded1])

		input_latent1 = Input(shape=(self.encoder_widths[-1],), name="Decoder_Input")
		decoded1 = Dense(units=self.decoder_widths[0],
			activation=None,
			kernel_regularizer=l2(l2_penalty),
			name="Decoder_Dense1")(input_latent1)
		decoded1 = LeakyReLU(alpha=alpha_val, name="Decoder_LeakyRelu1")(decoded1)
		for i,width in enumerate(self.decoder_widths[1:]):
			decoded1 = Dense(units=width,
				activation=None,
				kernel_regularizer=l2(l2_penalty),
				name="Decoder_Dense" + str(i + 2))(decoded1)
			decoded1 = LeakyReLU(alpha=alpha_val, name="Decoder_LeakyRelu" + str(i + 2))(decoded1)

		decoded1 = Dense(units=int(decoder_outdim),
			activation='relu',
			kernel_regularizer=l2(l2_penalty),
			name="Decoder_Dense" + str(len(self.decoder_widths) + 1))(decoded1)
	...
	def train_net(self):
		...
		history = self.network.fit(x=train_data, y=train_target,
		...
		modalpha1 = Input(shape=(self.encoder_widths[-1],), name="Alpha_Input")
		modnegalpha1 = Input(shape=(self.encoder_widths[-1],), name="Negalpha_Input")
		final_spec_1 = Input(shape=(decoder_outdim,), name="Track1_Input")
		final_spec_2 = Input(shape=(decoder_outdim,), name="Track2_Input")
		...
		#plt.figure(1)
		#plt.plot(np.arange(self.n_epochs)+1,history.history['loss'])
		#plt.plot(np.arange(1,self.n_epochs+1),history.history['val_loss'])
		#plt.xlabel('Epoch')
		#plt.ylabel('Loss (mse)')
		#plt.xlim([1,self.n_epochs])
		#plt.xticks(np.arange(1,self.n_epochs+1))
		#plt.legend(['loss', 'val_loss'], loc='upper right')
		#plt.savefig('figures/' + self.filename_out + '_loss.pdf',format='pdf')
